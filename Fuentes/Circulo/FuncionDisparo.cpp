...
long double FuncionDisparo::Calcula(const long double T, const bool FI)
{
   Error_calculo = 0;
#ifdef NM_FISICA 
   long double tn, g, u1, u2, r1;

   r1 = P[7]*(cosl(P[6])-cosl(atanl((2*P[0])/(P[2]*P[3]*P[3]))));
   if(P[1] >= r1) return 0.0;
   u1 = P[2] * P[2] * P[3] / 2.0;
   u2 = acosl(cosl(P[6])+((P[1]/P[7])*sinl(P[5]*T)));
   if (P[0] < u1) g = (P[3]*P[3]*P[2]/2.0)*tanl(u2); 
   if (P[0] >= u1) {
       if (u2>atanl(P[2]/P[3])) g = (P[2]*P[2]*P[3])-((P[2]*P[2]*P[2])/(2.0*tanl(u2)));   
        else g = (P[3]*P[3]*P[2]/2)*tanl(u2);
   }
   tn = (P[0] / P[4]) -(g / P[4]) + T;
   return tn;
#endif   
...
long double FuncionDisparo::Beta(const long double xx)
{
   ...
   if (P[0] >= 0.0 && P[0] < 1.0) {
      ...
      if (xx >= 0.0 && xx < xd )      res = (0.5 + (((-1.0)/(2.0*M_PI))*atanl(sinl(2.0*M_PI*xx)/(P[0]-cosl(2.0*M_PI*xx)))));
      if (xx > xd && xx < 0.5)        res = (1.0 + (((-1.0)/(2.0*M_PI))*atanl(sinl(2.0*M_PI*xx)/(P[0]-cosl(2.0*M_PI*xx)))));
      if (xx > 0.5 && xx < (1.0 -xd)) res = ((((-1.0)/(2.0*M_PI))*atanl(sinl(2.0*M_PI*xx)/(P[0]-cosl(2.0*M_PI*xx)))));
      if (xx > (1.0 -xd) && xx < 1.0) res = (0.5 + (((-1.0)/(2.0*M_PI))*atanl(sinl(2.0*M_PI*xx)/(P[0]-cosl(2.0*M_PI*xx)))));
      if (xx == xd)                   res = (3.0/4.0);
      if (xx == (1.0 - xd))           res = (5.0/4.0);
      if (xx == 0.5)                  res = 1.0;
   }
   if (P[0] > 1.0) {
      if (xx >= 0.0 && xx < 0.5) res = (1.0 + (((-1.0)/(2.0*M_PI))*atanl(sinl(2.0*M_PI*xx)/(P[0]-cosl(2.0*M_PI*xx)))));
      if (xx >= 0.5 && xx < 1.0) res = (((-1.0)/(2.0*M_PI))*atanl(sinl(2.0*M_PI*xx)/(P[0]-cosl(2.0*M_PI*xx))));
   }
   return (res);
