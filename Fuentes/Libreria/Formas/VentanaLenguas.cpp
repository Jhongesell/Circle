...
#define CLAVE_ENCABEZADO "ARLD16"  // ARLD16
//                                            ^^
//                                            ||__ Parámetros
//                                            |___ Versión
...
__fastcall TFormaLenguas::TFormaLenguas(TComponent* Owner) : TForm(Owner)
{
   ...
   pix = new Ventana_256;
   pix->Inicializa(Pix_x, Pix_y,255,Dim_Vtn,NUMERO_COLORES_48);
...
// Calculo de Lenguas   
   ...
   Factor_usado_zona_difeomorfismos = 1;
...
///////////////////////////////////////////////////////////////////////////   
   ...
   strcpy(Comentarios,"Generate for program in windows");
}
...
void __fastcall TFormaLenguas::FormResize(TObject *Sender)
{
   ...
   Escala.X = VTLenguas->Width / (long double) (Dim_Vtn.Xf - Dim_Vtn.Xi);
   Escala.Y = VTLenguas->Height / (long double) (Dim_Vtn.Yf - Dim_Vtn.Yi);
   // Grafica el contenido de la ventana
   Grafica();        
}     

// Al visualizar la ventana
...
void TFormaLenguas::Grafica(void)
{
   ...
   for (y = 0; y < Pix_y; y++) {
       for (x = 0; x < Pix_x; x++) {
           ...
           if (i != 255) VTLenguas->Canvas->Pixels[x * esc.X][y * esc.Y] = pix->Retorna_color(i);
       }
   }
}
...
void TFormaLenguas::RetornarValoresConfiguracion(void)
{
   ...
   // Calcular
   ////////////////////////////////////////////////////////
   AnsiString txt5(configura->EditA1->Text);
   ItsTrans = atoi(txt5.c_str());
   AnsiString txt6(configura->EditA2->Text);
   Num_max_trans = atoi(txt6.c_str());
   AnsiString txt7(configura->EditA3->Text);
   SizeOrbits = atoi(txt7.c_str());
   AnsiString txt8(configura->EditA4->Text);
   NCondInits = atoi(txt8.c_str());
   AnsiString txt9(configura->EditA5->Text);
   Tolerancia = atof(txt9.c_str());
   AnsiString txt10(configura->EditA6->Text);
   Ptos_X = atoi(txt10.c_str());
   AnsiString txt11(configura->EditA7->Text);
   Ptos_Y = atoi(txt11.c_str());
   /////////////////////////////////////////////////////////
   // Al terminar
   /////////////////////////////////////////////////////////
   pix->Cambia_dimension(Dim_Vtn, 255);
...
void __fastcall TFormaLenguas::MenuArchivoLoadFileofTonguesClick(TObject *Sender)
{
   ...
   if (Leer_Encabezado()) return;
          ...
          if (A >= Dim_Vtn.Xi && A <= Dim_Vtn.Xf && B >= Dim_Vtn.Yi && B <= Dim_Vtn.Yf) {
             for (xi = 0; xi < SizeTgs; xi++) {
                if (Tongues_P[xi] == xarreglo_sincronizaciones_P && Tongues_Q[xi] == xarreglo_sincronizaciones_Q) {
                   pix->Asigna_valor(A,B,xi%47);
                   px = (A - Dim_Vtn.Xi) * Escala.X;
                   py = VTLenguas->Height - (((B - Dim_Vtn.Yi) * Escala.Y) + 1.0);
                   VTLenguas->Canvas->Pixels[px][py] = pix->Retorna_color(xi%47);
                   break; 
                }
             }
          }
    ...
    Ayuda->Editor->Lines->Add(xcad1);
    Ayuda->Editor->Lines->Add("");
    Ayuda->Editor->Lines->Add("   Remark:");
    Ayuda->Editor->Lines->Add(Comentarios);
    Ayuda->Show();
...
void __fastcall TFormaLenguas::LoadFileofTonguesTXT1Click(TObject *Sender)
{
   if (!ConvertirArchivo->Execute()) return;
   strcpy(Archivo_lenguas,ConvertirArchivo->FileName.c_str());
///////////////////////////////
   ...
   if (Leer_Encabezado()) return;
    ...
    while (1)
    {
       ...
       if (Indice_multiestabilidad == 0) {
          ...
          pix->Asigna_valor(A,B,Indice_sincronizaciones%47);
          px = (A - Dim_Vtn.Xi) * Escala.X;
          py = VTLenguas->Height - (((B - Dim_Vtn.Yi) * Escala.Y) + 1.0);
          VTLenguas->Canvas->Pixels[px][py] = pix->Retorna_color(Indice_sincronizaciones%47);
        } else {
...
int TFormaLenguas::Leer_Encabezado(void)
{
   ...
   // Comentarios
   fread(Comentarios, 500, 1, InField);
   Comentarios[500] = 0;
...
//   if (MessageBox(Handle,"Do you wish escalate window?","Toungues scenery",MB_YESNO + MB_ICONQUESTION)  == IDYES) {
      Dim_Vtn.Xi = xMinX;
      Dim_Vtn.Xf = xMaxX;
      Dim_Vtn.Yi = xMinY;
      Dim_Vtn.Yf = xMaxY;
...
//   }
   ...
   fread(&t1, sizeof(t1), 1, InField);
   // Valor de los parámetros
   long double parm[NUM_MAX_PARAMETROS];
   for (i = 0; i < t1; i++) {
      fread(&t2, sizeof(t2), 1, InField);
      parm[i] = t2;
   }
   ...
   fread(&t1, sizeof(t1), 1, InField);
   for (i = 0; i < t1; i++) {
      fread(&t3, sizeof(t3), 1, InField);
      if (i == 0) Primer_Eje = t3;
      if (i == 1) Segundo_Eje = t3;
   }
   
   // Número de parámetros de cálcular
   fread(&t1, sizeof(t1), 1, InField);
   for (i = 0; i < t1; i++) {
      fread(&t2, sizeof(t2), 1, InField);
   }
...
i	*Puntos en X
i	*Puntos en Y
i	*Número de Parámetros
[f]	*Valor de los parámetros
i	*Número de parametros a bifurcar
[i]	*Números de parámetros sobre los que se bifurca
i	Número parametros de cálculo 
[f]	Parametros de cálculo 
//////////////
f	X
f	Y
i	Número de sincronizaciones encontradas
f	P's
f	Q's
*/
   ...
   // Encabezado
   fwrite(CLAVE_ENCABEZADO, 6, 1, OutField);

   //Comentarios
   fwrite(Comentarios, 500,1, OutField);
   ...
   // Número de parámetros y valores de estos
   t1 = fun_disp.Numero_parametros;
   fwrite(&t1, sizeof(t1), 1, OutField);       // Número de parámetros
   long double parm[NUM_MAX_PARAMETROS];
   fun_disp.RetornaParametros(parm);   
   for (int i = 0; i < t1; i++) {
      t2 = parm[i];
      fwrite(&t2, sizeof(t2), 1, OutField);    // Valor de los parámetros 
   }

   // Número de parámetros a bifurcar y cuales son
   t1 = 2;
   fwrite(&t1, sizeof(t1), 1, OutField);       // Número de parámetros a bifurcar
   t1 = Primer_Eje;
   fwrite(&t1, sizeof(t1), 1, OutField);       // Primer parámetro a bifurcar
   t1 = Segundo_Eje;
   fwrite(&t1, sizeof(t1), 1, OutField);       // Segundo parámetro a bifurcar
   
   // Número de parámetros de cálculo
   t1 = 0;
   fwrite(&t1, sizeof(t1), 1, OutField);       // Número de parámetros a bifurcar
   // Falta detallar estos ....
...
void TFormaLenguas::Calcula_Lenguas_Solicitadas(const long double a, const long double b, const long double aaux, const long double baux, const bool difeomorfismo)
{
   ...
   if (Indice_sincronizaciones > 0)
   {
      ...
      for (i = 0; i < Indice_sincronizaciones; i++)
      {
         ...
         pix->Asigna_valor(a,b,Count%47);
         px = (a - Dim_Vtn.Xi) * Escala.X;
         py = VTLenguas->Height - (((b - Dim_Vtn.Yi) * Escala.Y) + 1.0);
         VTLenguas->Canvas->Pixels[px][py] = pix->Retorna_color(Count%47);
      }     
...
// Eror al hacer zoom si hay mas de dos parametros, pierde las variables del las lenguas seleccionadas
